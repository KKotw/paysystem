...
namespace PaySystem.DLL.Classes
{
    public class CPayout
    {
            ...
            for (i = 1; i < responseLength; ++i)
            {
                switch (response[i])
                {
                    ...
                    case CCommands.SSP_POLL_CREDIT:
                        ...
                        price = CHelpers.FormatToCurrency(data.Value);
        ...
        /// <returns>   </returns>
        public int ChangeCashCount(int payout)
        {
            return (payout / 10)+ (payout % 10) / 5 + (payout % 10) % 5;
        }
        ...
        /// </summary>
        /// <param name="payout"></param>
        /// <returns>is cash enough to payout </returns>
        
        public bool IsCashEnough(int payout)
        {
            int isten = 0;
            int isfive = 0;
            int isone = 0;
            ...
            foreach (ChannelData d in m_UnitDataList)
            {
                if (d.Value == 100 && d.Level < isone)
                {
                    return false;
                }
                else if (d.Value == 500 && d.Level < isfive)
                {
                    return false;
                }
                else if (d.Value == 1000 && d.Level < isten)
                {
                    return false;
                }
        ...
        //set channel  to cashbox or payout 
        /// <summary>
        ///  1：5---- 5：30 ----10：30
        /// </summary>
        /// <returns></returns>
        public void AdminSetChannelToPayout(TextBox log)
        {
            ...
            foreach (ChannelData d in m_UnitDataList)
            {
                ...
                bool ispayout = false;
                 if (d.Value == 500 && d.Level < 30)
                {
                    ispayout = true;
                }
                else if (d.Value == 1000 && d.Level < 30)
                {
                    ispayout = true;
                }
                else   //>=20  to cashbox
                ...
                if (ispayout)
                    ChangeNoteRoute(d.Value, d.Currency, false, log); //payout
                else
                   ChangeNoteRoute(d.Value, d.Currency, true, log); //cashbox
            }
        ...
        /// </summary>
        /// <returns></returns>
        public void SetChannelToPayout(TextBox log)
        {
            ...
            foreach (ChannelData d in m_UnitDataList)
            {
                ...
                bool ispayout = false;
                if (d.Value == 100 && d.Level < 5)
                {
                    ispayout = true;
                }
                else if (d.Value == 500 && d.Level < 25)
                {
                    ispayout = true;
                }
                else if (d.Value == 1000 && d.Level < 25)
                {
                    ispayout = true;
                }
                else if (d.Value == 2000 && d.Level < 5) //退钱使用
                {
                    ispayout = true;
                }
                else if (d.Value == 5000 && d.Level < 1) //退钱使用
                {
                    ispayout = true;
                }
                else if (d.Value == 10000 && d.Level < 1) //退钱使用
                {
                    ispayout = true;
                }
                else   //>=50  to cashbox
                    ...
                    switch (cmd.ResponseData[0])
                    {
                        case CCommands.SSP_RESPONSE_CMD_CANNOT_PROCESS:
                            ...
                            else
                            {
                                ...
                                flag = "ERR." + cmd.ResponseData[1].ToString();
                            }
                            return false;
